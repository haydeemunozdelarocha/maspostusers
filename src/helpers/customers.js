...
export function getInventario(pmb, status, timeframe) {
    return axios.get(`${process.env.REACT_APP_MASPOST_SOURCE}recepcion/all`, {responseType: 'json', params: {pmb: 160, status: status, date: timeframe}});
}

export function getAutorizados(pmb) {
    return axios.get(`${process.env.REACT_APP_MASPOST_SOURCE}clientes/autorizados`, {responseType: 'json', params: {pmb: 103}});
}
